...
def logistic_regression_test():
    ...
    # 임의의 (x, y) 2차원 백터(좌표)를 100개 생성
    y = (X[:, 0] + X[:, 1]>0).astype(int)
    # x 좌표와 y 좌표를 더해 0보다 크면 1, 아니면 0
    ...
    # 회귀 분석이란?
    # 결론적으로 Y = A.X + B 를 찾는 수학적 기법이라 볼 수 있음
    ...
    # 테스트용 좌표를 가지고 결과값을 주문
    # X = [(1, 2), (3,4), (2, 3), ...]
    # y= [   1  ,   1  ,   1   , ...]
    y_pred = model.predict(X_test)

    # 예측치인 pred 값과 실제 테스트 셋인 test를 비교하여
    # 정확도가 어느정도인지 판별
